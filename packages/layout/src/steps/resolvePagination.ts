...
import { omit, asyncCompose } from '@react-pdf/fns';
...
const resolveDynamicPage = async (
...
const resolvePagination = async (
  ...
  await Promise.all(
    root.children.map(async (page, pageIndex) => {
      let subpages = await paginate(page, pageNumber, fontStore, root.yoga);
